...
const batchProjectCentering = document.getElementById('batchProjectCentering');
const batchProjectAlignment = document.getElementById('batchProjectAlignment');
const state = {
    cursor: { x: 0, y: 0 },
    hexagonColumns: 0
};
...
async function loadProjects() {
    const projectsData = await fetch('resources/projects.json', { cache: 'no-cache' }).then(response => response.json());
    let counter = 0;
    for (let [title, description] of Object.entries(projectsData.complete)) {
        const projectWrapper = projectTemplate.content.querySelector('.projectWrapper').cloneNode(true);
...
function updateHexagons() {
    ...
    if (hexagonColumns == state.hexagonColumns) {
        return;
    }
